...
def consultar_meu_progresso(usuario):
    ...
    if Progresso.objects.filter(usuario=usuario, modulo__numero=4).exists() or Tentativa.objects.filter(
        ...
        numeros.append(4)
    if Progresso.objects.filter(usuario=usuario, modulo__numero=5).exists() or Tentativa.objects.filter(
        usuario=usuario, palavra__modulo__numero=5
    ).exists():
        numeros.append(5)
    if Progresso.objects.filter(usuario=usuario, modulo__numero=6).exists() or Tentativa.objects.filter(
