...
class Palavra(models.Model):
    ...
    imagem = models.ImageField(
        upload_to='palavras/imagens/',
        blank=True
    )
    audio = models.FileField(
        upload_to='palavras/audios/',
        blank=True
    )
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(
                fields=['modulo', 'palavra'],
                name='palavra_unica_por_modulo'
            ),
            models.UniqueConstraint(
                fields=['modulo', 'ordem'],
                name='ordem_palavra_unica_por_modulo'
            ),
        ]
...
class Comparacao(models.Model):
    ...
    palavra_base = models.ForeignKey(
        Palavra,
        on_delete=models.PROTECT,
        related_name='comparacoes_como_base'
    )

    palavra_comparada = models.ForeignKey(
        Palavra,
        on_delete=models.PROTECT,
        related_name='comparacoes_como_destino'
    )
    ...
    def clean(self):
        ...
        if (
            ...
            erros['palavra_comparada'] = (
                'A palavra comparada deve ser diferente '
                'da palavra base.'
            )
    ...
    class Meta:
        constraints = [
            models.UniqueConstraint(
                ...
                name="progresso_unico_por_usuario_modulo"
            ),

            models.CheckConstraint(
                check=models.Q(percentual__lte=100),
                name="progresso_percentual_ate_100"
            ),
        ]
...
class Sessao(models.Model):
    ...
    class Meta:
        ...
        constraints = [
            models.CheckConstraint(
                check=(
                    models.Q(finalizada_em__isnull=True)
                    |
                    models.Q(
                        finalizada_em__gte=models.F(
                            "iniciada_em"
                        )
                    )
                ),
...
class Tentativa(models.Model):
    ...
    resposta_reconhecida = models.CharField(
        max_length=255,
        blank=True
    )
    ...
    resultado = models.CharField(
        ...
        default=Resultado.NAO_RECONHECIDO
    )
        ...
        if self.sessao_id and self.usuario_id:
            if not Sessao.objects.filter(
                ...
                raise ValidationError({
                    "sessao":
                        "A sessão deve pertencer "
                        "ao usuário da tentativa."
                })
