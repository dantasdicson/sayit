...
class Modulo8Tests(TestCase):
    @classmethod
    def setUpTestData(cls):
        call_command('popular_sayit', stdout=StringIO())
        cls.usuario = get_user_model().objects.create_user(username='modulo8', email='m8@test.com')
        cls.modulo = Modulo.objects.get(numero=8)
    ...
    def url(self, ordem):
        return reverse('descoberta_modulo', args=[8, ordem])
    ...
    def acertar(self, indice, segunda=False, texto=None):
        ...
        return self.client.post(reverse('registrar_acerto', args=[8]), {
    ...
    def test_quatro_etapas_dois_acertos_independentes_e_persistencia(self):
        for indice in range(4):
            ...
            destino = self.url(indice + 2) if indice < 3 else reverse('resumo_modulo_8')
    ...
    def test_bloqueio_erro_silencio_e_recarregamento(self):
        ...
        self.assertEqual(self.client.get(reverse('resumo_modulo_8')).status_code, 409)
        self.assertEqual(self.client.get(reverse('conclusao_modulo_8')).status_code, 409)
        for texto in ('wrong', '', ' ... ', 'photo', 'wrong phone'):
            self.assertEqual(self.acertar(0, texto=texto).status_code, 400)
        self.assertFalse(Tentativa.objects.exists())
        self.assertEqual(self.acertar(0, texto=' PHONE! ').status_code, 200)
        self.client.logout()
        self.client.force_login(self.usuario)
    ...
    def test_resumo_conclusao_e_modulo_9_disponivel(self):
        ...
        with patch('core.views.default_storage') as storage:
            ...
            storage.url.return_value = '/media/modulos/8/resumo.mp3'
            resposta = self.client.get(reverse('resumo_modulo_8'))
        for texto in ('O som PH', '/media/modulos/8/resumo.mp3', 'resumo_audio.js'):
    ...
    def test_midias_reais_associacao_e_renderizacao(self):
        ...
        call_command('associar_imagens', modulo=8, stdout=StringIO())
        call_command('associar_audios', modulo=8, stdout=StringIO())
        ...
        self.assertEqual(Audios.validar_mp3(Path(settings.MEDIA_ROOT) / 'modulos/8/resumo.mp3')[0], 'valido')

    def test_autenticacao_csrf_e_catalogo_incompleto(self):
        cliente = Client(enforce_csrf_checks=True)
        cliente.force_login(self.usuario)
        self.assertEqual(cliente.post(reverse('conclusao_modulo_8')).status_code, 403)
        self.client.logout()
        for url in (self.url(1), reverse('explicacao_modulo_8'),
                    reverse('resumo_modulo_8'), reverse('conclusao_modulo_8')):
            self.assertEqual(self.client.get(url).status_code, 302)
        self.client.force_login(self.usuario)
        self.pares[-1].delete()
        self.assertEqual(self.client.post(reverse('conclusao_modulo_8')).status_code, 409)
