...
for (const [index, pair] of [['moon', 'book'], ['food', 'good'], ['room', 'look'], ['school', 'foot']].entries()) {
  test(`Módulo 9 par ${index + 1}: mantém os sons de OO distintos e exige os dois acertos`, async () => {
    const url = index === 3 ? '/modulos/9/resumo/' : `/modulos/9/descobertas/${index + 2}/`;
    const app = setup(pair, {module: 9, url});
    await app.say(0, pair[1]); assert.equal(app.requests.length, 0); app.locked();
    await app.say(0, ` ${pair[0].toUpperCase()}! `); app.locked();
    await app.say(1, pair[0]); assert.equal(app.requests.length, 1); app.locked();
    await app.say(1, pair[1]); assert.equal(app.next.disabled, false);
    assert.equal(app.requests[0].url, '/modulos/9/progresso/acertos/');
    app.next.click(); assert.deepEqual(app.navigations, [url]);
  });
}
